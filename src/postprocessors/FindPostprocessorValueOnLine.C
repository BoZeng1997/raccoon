...
#include "FindPostprocessorValueOnLine.h"
...
registerMooseObject("raccoonApp", FindPostprocessorValueOnLine);

InputParameters
FindPostprocessorValueOnLine::validParams()
{
  ...
  params.addRequiredParam<PostprocessorName>("pp_name", "Post-processors name");
...
FindPostprocessorValueOnLine::FindPostprocessorValueOnLine(const InputParameters & parameters)
  : GeneralPostprocessor(parameters),
    Coupleable(this, false),
    _start_point(getParam<Point>("start_point")),
    _end_point(getParam<Point>("end_point")),
    _length((_end_point - _start_point).norm()),
    _target(getPostprocessorValue("pp_name")),
...
void
FindPostprocessorValueOnLine::initialize()
{
...
void
FindPostprocessorValueOnLine::execute()
{
  Real s;
  Real s_left = 0.0;
...
Real
FindPostprocessorValueOnLine::getValueAtPoint(const Point & p)
{
...
PostprocessorValue
FindPostprocessorValueOnLine::getValue() const
{
  return _position;
}
